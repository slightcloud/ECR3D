...
N_grid_x = round((R-(-R))/0.0005+1);   %x方向的格点数
N_grid_y = round((R-(-R))/0.0005+1);   %y方向的格点数
N_grid_z = round((Z-0)/0.0005+1);      %z方向的格点数
...
for ts = 1:step_num_i
   ...
   %求出每个格点上的电势
   phi = GS_SOR_3D(den,N_grid_x,N_grid_y,N_grid_z,tol,0.7,1);
